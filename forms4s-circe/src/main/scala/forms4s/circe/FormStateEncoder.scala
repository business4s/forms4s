...
object FormStateEncoder {
  ...
  def encode(element: FormElementState): Json = element match {
    ...
    case FormElementState.Group(_, values, _)     => Json.fromFields(values.map(elem => elem.id -> encode(elem)))
    case FormElementState.Multivalue(_, elems, _) => Json.fromValues(elems.map(encode))
  }
